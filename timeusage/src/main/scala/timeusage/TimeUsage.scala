...
object TimeUsage {
  ...
  def classifiedColumns(columnNames: List[String]): (List[Column], List[Column], List[Column]) = {
    def isPrimaryNeeds(col : String) : Boolean =
        col.startsWith("t01") ||
        col.startsWith("t03") ||
        col.startsWith("t11") ||
    ...
    def isWorkingActivities(col : String) : Boolean =
        col.startsWith("t05") ||
        col.startsWith("t1805")

    def isOtherActivities(col : String) : Boolean =
        col.startsWith("t02") ||
        col.startsWith("t04") ||
        col.startsWith("t06") ||
        col.startsWith("t07") ||
        ...
        col.startsWith("t15") ||
        col.startsWith("t16") ||
        (col.startsWith("t18") && !isPrimaryNeeds(col) && !isWorkingActivities(col))

    (columnNames.filter(isPrimaryNeeds).map(column), columnNames.filter(isWorkingActivities).map(column), columnNames.filter(isOtherActivities).map(column))
  }
